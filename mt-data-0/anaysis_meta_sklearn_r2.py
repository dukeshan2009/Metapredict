...
for k in range(len(file)):
    ...
    a = list(df['PLDDT'])
    b = list(df['MT-Pre'])
    r2 = r2_score(np.array(a),np.array(b),multioutput='variance_weighted', force_finite=False)
    r2_total.append(r2)
...
r2_total_residue = r2_score(np.array(AF2_plddt_total),np.array(Meta_plddt_total),multioutput='variance_weighted', force_finite=False)
...
plt.savefig(os.path.dirname(__file__) + '/' + path.split('\\')[-1] + '_sklearn_r2.png')
